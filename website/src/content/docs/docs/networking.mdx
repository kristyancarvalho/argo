...
description: Understand Argo NetworkManager state, scheduler priority, optional RX shaping, and adaptive traffic policies.
---
...
| `background` | Starts at the minimum, reclaims idle capacity gradually, and yields faster  |
...
The background policy uses the same measured RX ceiling but has a distinct conservative lifecycle. It starts at `min_rate` whenever activated, increases only after consecutive healthy samples, and decreases more aggressively after confirmed target violations. Missing telemetry also moves it toward the minimum. This makes background a feedback policy, not a fixed low cap or an alias for `latency`.

No receiver can remove delay intrinsic to a remote path. If measured latency remains above the target when Argo is already at `min_rate`, status continues to expose that condition instead of claiming that the target is satisfied.
